...
<div class="dashboard-body">
    ...
    <div class="breadcrumb-with-buttons mb-24 flex-between flex-wrap gap-8">
        ...
        <!-- Export Button (Green) & Add Room Button -->
        <div class="d-flex gap-3">
            <a href="{{ route('rooms.export') }}" class="btn btn-success btn-sm">Export as Excel</a>
            <a href="{{ route('rooms.create') }}" class="btn btn-primary btn-sm">Add Room</a>
        </div>
    </div>
    ...
    <div class="card mt-24 mb-24">
        <div class="card-body">
            <form action="#" class="search-input-form" id="roomFilterForm">
                ...
                <div class="search-input">
                    <input type="text" id="filterRoomNumber" class="form-control h6 rounded-4 mb-0 py-6 px-8" placeholder="Search by Room Number">
                </div>
                <div class="search-input">
            ...
            <tbody>
                @foreach ($rooms as $key => $room)
                    <tr>
                        ...
                        <td class="h6 mb-0 fw-medium text-gray-300">
                            <a href="{{ route('rooms.edit', $room->id) }}" class="bg-main-50 text-main-600 py-2 px-14 rounded-pill hover-bg-main-600 hover-text-white">Edit</a>
                            <form action="{{ route('rooms.destroy', $room->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-danger-50 text-danger-600 py-2 px-14 rounded-pill hover-bg-danger-600 hover-text-white" onclick="return confirm('Are you sure you want to delete this room?')">Delete</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
